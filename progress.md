...
## Session: 2026-08-02 (continued) — DCode ACP client + runtime/adapters restructure
- Probed `dcode --acp` (deepagents-code 0.1.51) empirically to learn its real ACP wire shape: `initialize` → `session/new` (or `session/load` to resume) → `session/prompt` → `session/update` notifications (`agent_message_chunk`, `tool_call`, `tool_call_update`) → prompt response `{stopReason}`; server can also push `session/request_permission`, `fs/read_text_file`, `fs/write_text_file`. `session/cancel` is a notification (no response) used for abort.
- Restructured `packages/acp-gateway/src/` from a flat file layout into `runtime/` (transport-agnostic run/session state machine: `NormalizedAgentEvent.ts`, `RunController.ts`, `SessionController.ts`, `wire.ts`) and `adapters/` (one directory per agent wire protocol):
  - `adapters/codex/CodexJsonAdapter.ts` — the old `process-manager.ts` + `codex-event-mapper.ts` logic, unified behind the new `AgentAdapter` interface (line-delimited JSON on stdout).
  - `adapters/dcode/DCodeAcpAdapter.ts` — new: a real JSON-RPC 2.0 ACP client over stdio for `dcode --acp`, implementing the handshake/session/prompt/cancel/permission flow found above. One long-lived process per agent, reused across turns (matches the existing Codex model).
  - `adapters/registry.ts` — `AdapterRegistry` picks `DCodeAcpAdapter` vs `CodexJsonAdapter` per-agent from `AgentConfig.adapter` (`"acp" | "json"`, inferred from `type` when unset).
- `protocol.ts` gained the `AgentAdapter` discriminator and `resolveAdapter()`; `rpc.ts`/`server.ts`/`index.ts` updated to depend only on the adapter-agnostic `RunController`/`SessionController`/`AdapterRegistry`, no agent-specific code left in the transport layer.
- Confirmed via grep that nothing still imports the old flat `process-manager.ts`/`codex-event-mapper.ts` (their logic is fully isolated inside `CodexJsonAdapter`, not discarded) and `git rm`'d both orphaned files.
- Added `packages/acp-gateway/config/agents.json` — DCode registered first/default (`adapter: "acp"`), Codex second (`adapter: "json"`); `index.ts`'s built-in defaults already matched this shape and are used as a fallback when the file is absent.
- Added `scripts/test-gw-dcode-2turn.cjs` — same two-turn-same-session repro pattern as `test-gw-2turn.cjs` but against `agentId: "dcode"`, plus an `agents.list` pid check between turns to assert the ACP subprocess is reused (not respawned) across turns.
- Verified: `pnpm typecheck` ✓, `pnpm test` ✓ (7/7 vitest, db suite unaffected by restructure), DCode 2-turn script ✓ — turn 1 "ALPHA" and turn 2 "BETA" both streamed correctly, both reached `state: final`, same pid (`26016`) reported by `agents.list` after both turns (`SAME PROCESS ACROSS TURNS: true`). DB inspection after the run showed both `runs` rows with `agent_id: "dcode"`, `status: "completed"`, real `prompt`/`response` text, and the same `session_id`.
- Browser verification: **not completed this session** — no browser-automation tool (Interceptor or otherwise) was available in this environment/session. Confirmed instead via the same WebSocket protocol the browser UI drives (`chat.send`/`session/update` event relay, verified end-to-end by the DCode 2-turn script) plus a raw `curl` of `/` showing the Next.js SPA shell serves correctly. Manual verification steps for a human/next session: `ACP_AGENTS_CONFIG=packages/acp-gateway/config/agents.json pnpm --filter @agent-workspace/acp-gateway dev`, open the served UI, pick the "DCode" thread, and send a message.
- **Next**: Phase I (persistent apps/artifacts) still not started. DCode adapter is now verified end-to-end at the protocol/process level and set as the default agent; UI-level (browser) confirmation of the DCode thread is the next concrete verification gap.
